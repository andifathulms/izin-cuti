...
9. **Nothing is transmitted, stored remotely, or measured.** No analytics, no error reporting, no beacon, no fetch of any kind at runtime. Profiles and mappings live in local storage with explicit export and clear-all. A template may additionally be kept in IndexedDB, **opt-in only** — never by default, always with the date it was remembered on screen, and cleared by clear-all. What belongs there is a blank copy, and the UI says so.

9a. **A blank copy is produced by the fill engine, never by Word.** Emptying values is not offered: a cell holding nothing but whitespace is how a checkbox target is identified, so an emptied value alone in a cell reclassifies its cell and shifts every index after it. Values become their field names. The copy and its re-fingerprint happen in one step, because blanking one value moves the context of another in the same paragraph.

10. **No *filled* official template ships in the repository.** Test fixtures are synthetic documents generated by the test suite. One **blank** form ships — `presets/formulir-cuti-kosong.docx`, produced by `pnpm blank` from a filled original that is gitignored and never committed. It carries no personal data belonging to anyone, the suite asserts that against the exact values the original held, and it is the only real document here. A filled form, anyone's, still never lands in this repository.
